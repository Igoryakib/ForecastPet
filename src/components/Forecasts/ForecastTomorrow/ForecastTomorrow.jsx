...
const ForecastTomorrow = function () {
  ...
  const temp =
    unit === "C"
      ? Math.round(weatherToday.temp.day)
      : Math.round(convertUnitFn(weatherToday.temp.day));
  // console.log(weatherToday)

  // function to define current ico
  const weatherIconFn = function () {
    if (icon.startsWith("01")) return sunnyIcon;
    if (icon.startsWith("02")) return cloudyIcon;
    if (icon.startsWith("03")) return cloudyIcon;
    if (icon.startsWith("04")) return cloudyIcon;
    if (icon.startsWith("09")) return cloudyIcon;
    if (icon.startsWith("10")) return rainyIcon;
    if (icon.startsWith("11")) return rainyIcon;
    if (icon.startsWith("13")) return snowyIcon;
    if (icon.startsWith("50")) return cloudyIcon;
  };
  ...
  return (
    <section className={styles.section}>
      ...
      <p className={styles.temp}>{temp === 0 ? "0°" : temp + "°"}</p>
