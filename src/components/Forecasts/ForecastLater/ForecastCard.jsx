...
const ForecastCard = function({tempMax = 30, tempMin = 20, icon = 'rainy', date = '1 August'}){
  const currentIcon = [icon_cloudyRainy, icon_cloudySnowy, icon_cloudyStorm, icon_partlyCloudy, icon_rainy, icon_snowy, icon_sunny].filter(i => i.split('__')[1].startsWith(icon) )

  const options = {
    day: 'numeric',
    month: 'long'
  }
  const fullDateArray = Intl.DateTimeFormat('uk', options).format(new Date(date)).split(' ');
  ...
  return (
    <figure className={styles.card} >
      ...
      <div className={styles.temp}>
        <span className={styles.tempMax}>{tempMax === 0 ? "0" : tempMax > 0 ? `+${tempMax}°` : `-${tempMax}`}</span>
        <span className={styles.tempMin}>/{tempMin === 0 ? "0" : tempMin > 0 ? `+${tempMin}°` : `-${tempMin}`}</span>
      </div>
