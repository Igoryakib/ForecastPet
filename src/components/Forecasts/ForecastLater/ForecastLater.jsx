...
const ForecastLater = function () {
  ...
  return (
    <section className={styles.section}>
      ...
      {isMoreDays ? TEMPORARY_DATA.map(day => <ForecastCard key={day.date} date={day.date} tempMax={day.tempMax} tempMin={day.tempMin} icon={day.icon} />) : TEMPORARY_DATA.slice(0, 3).map(day => <ForecastCard key={day.date} date={day.date} tempMax={day.tempMax} tempMin={day.tempMin} icon={day.icon} />)}
      <Time />
      {/* </div> */}
    </section>
  );
